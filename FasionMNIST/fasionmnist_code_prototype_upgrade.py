...
def update_epoch_accuracy_matrix_dataset1(test_acc, current_epoch_accuracy_detail_lst, train_dataset, trainset_permutation_inds, args):
    ...
    if threshold_epoch != None and test_acc >= threshold_test_accuracy_to_build_new_data and filter_button == True:
        ...
        ratio = (min_max_data_nums_per_class[1] * 10 / epoch_accuracy_matrix.shape[0])
        visited_indexes = set()
        pop_num = 50
        ...
        last_len_vis = 0
        while len(visited_indexes) < int(min_max_data_nums_per_class[1] * 10):
            ...
            for i in range(sorted_angles.shape[0]):
                for k in range(sorted_angles[i].shape[0] - 1, 0, -1):
                    ...
                    if right_other in unforgettable_idx and right_me not in unforgettable_idx:
                        if len(visited_indexes) < int(min_max_data_nums_per_class[1] * 10): visited_indexes.add(right_me) 
                    elif right_other not in unforgettable_idx and right_me in unforgettable_idx:
                        if len(visited_indexes) < int(min_max_data_nums_per_class[1] * 10): visited_indexes.add(right_other) 
                    else: break
            print(f"len visited_indexes: {len(visited_indexes)} / {int(min_max_data_nums_per_class[1] * 10)}")
            ...
            if cnt_almost_same_times > max_almost_same_times:
                ...
                for i in range(min(pop_num, len(unforgettable_idx))):
                    if len(unforgettable_idx) > (1 - ratio) * epoch_accuracy_matrix.shape[0]:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--no_dropout', action='store_true', default=False, help='remove dropout')
    ...
    parser.add_argument("--max_data_nums_per_class", type=int, default=600, help="max_data_nums_per_class")
    ...
    '''
    parser.add_argument("--filtered_index_file_path", type=str, default="C:\\Users\\GM\\Desktop\\liuzhengchang\\CODE\\filtered_index_file_path_fasionmnist_10.pt", help="filtered_index_file_path")
    parser.add_argument("--sorting_file", type=str, default="C:\\Users\\GM\\Desktop\\fashionmnist_results\\fashionmnist_sorted.pkl", help="sorting_file")

    args = parser.parse_args()
    main(args)

# filter:
# python fasionmnist_code_prototype_upgrade.py --epochs 100 --batch_size 128 --lr 0.01 --seed 1 --th_win 3 --th_stable_acc 0.001 --th_stable_loss 0.001 --th_train_acc 0.85 --filter_button --threshold_occupation 2 --start_new_dataloader_min_nums 100 --threshold_test_accuracy_to_build_new_data 0.6  --min_data_nums_per_class 10 --max_data_nums_per_class 600 --run_mode filter --filtered_index_file_path C:\\Users\\GM\\Desktop\\liuzhengchang\\CODE\\filtered_index_file_path_fasionmnist_10.pt --sorting_file C:\Users\GM\Desktop\liuzhengchang\CODE\fasionmnist_results\fasionmnist_sorted.pkl